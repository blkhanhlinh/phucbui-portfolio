        ...
        <div className={classNames(styles.grid, "grid grid-cols-2 grid-rows-2 gap-x-32 gap-y-10")}>
            {projects.map((project) => (
                <figure key={project.id} className={styles.card}>
                    <div className={classNames(styles.cardImage, "w-[420px] h-[110px] rounded-small overflow-hidden")}>
                        <Image src={project.src} alt={project.name} width={420} height={110} className={classNames(styles.image, "rounded-small")}/>
                    </div>
                    <figcaption>
